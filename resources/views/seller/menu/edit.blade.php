<x-app-layout>
    ...
    <div class="space-y-6 p-4 sm:p-6 lg:p-8 max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 sm:p-8">
            <form action="{{ route('seller.menu.update', $menuItem) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Nama Menu</label>
                    <input id="name" type="text" name="name" value="{{ old('name', $menuItem->name) }}"
                ...
                <div>
                    ...
                    <input id="price" type="number" name="price" value="{{ old('price', $menuItem->price) }}"
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all duration-200"
                ...
                <div>
                    ...
                    <input id="stock" type="number" name="stock" value="{{ old('stock', $menuItem->stock) }}"
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all duration-200"
                        min="0" required>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const showPreview = (srcOrFile) => {
                ...
                if (typeof srcOrFile === 'string') { show(srcOrFile); return; }
